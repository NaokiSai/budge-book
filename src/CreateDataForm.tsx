...
import { DatePickerGroup } from './conponents/DatePickerGroup'
...
export const CreateDataForm = ({ data }: CreateDataFormProps) => {
	...
	const savehandle = async () => {
		...
		try {
			const response = await PostData(JSON.stringify(data)); // これも必要に応じて呼び出してください
		...
		<Stack direction="column" spacing={1} sx={{ width: 'fit-content', mx: 'auto', pt: 2 }}>
			...
			<Stack spacing={1} sx={{ backgroundColor: '#fff', border: 1, borderColor: 'divider', borderRadius: 2, boxShadow: 1, p: 2 }}>
				<Stack sx={{ width: '100%', display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
					<Typography sx={{ fontSize: 14, ml: 2, minWidth: 100 }}>金額 : </Typography>
					<TextField
						value={amount}
						...
						sx={{ width: 200, ml: 'auto', mr: 2 }} />
				</Stack>
				<Stack sx={{ width: '100%', display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
					<Typography sx={{ fontSize: 14, ml: 2, minWidth: 100 }}>支払者 : </Typography>
					<Select
						value={paymentPerson}
				...
				<Stack sx={{ width: '100%', display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
					<Typography sx={{ fontSize: 14, ml: 2, minWidth: 100 }}>カテゴリ : </Typography>
					<Select
						value={category}
				...
				<Stack sx={{ width: '100%', display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
					<Typography sx={{ fontSize: 14, ml: 2, minWidth: 100 }}>支払方法 : </Typography>
					<Select
						value={paymentMethod}
				...
				<Stack sx={{ width: '100%', display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
					<Typography sx={{ fontSize: 14, ml: 2, minWidth: 100 }}>立替有無 : </Typography>
					<Select
						value={isAdvance}
				...
				<Stack sx={{ width: '100%', display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
					<Typography sx={{ fontSize: 14, ml: 2, minWidth: 100 }}>購入店 : </Typography>
					<TextField
						value={purchaseStore}
				...
				<Stack sx={{ width: '100%', display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
					<Typography sx={{ fontSize: 14, ml: 2, minWidth: 100 }}>メモ : </Typography>
					<TextField
						value={memo}
